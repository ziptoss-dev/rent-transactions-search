...
- **LH 전세임대 매칭**: LH 전세임대 데이터와 실거래가 자동 매칭 및 필터링
  - LH 전세임대 체크박스로 해당 거래만 조회
  - LH 뱃지 표시 (입주자 유형, 방 개수, LH 지원금 툴팁)
  - LH 필터 시 전체 결과 한 번에 표시 (무한 스크롤 비활성화)
- **오피스텔 기준시가 검증**: 보증금이 기준시가 126%를 초과하는지 색상으로 표시 (초록/빨강)
- **건물별 상세 조회**: 건물명 클릭 시 해당 건물의 모든 실거래 내역을 모달로 확인
- **소유자 정보 조회**: VWorld API 연동으로 건물별 소유자 정보 확인 (동·호별 그룹화)
- **무한 스크롤**: 스크롤 시 자동으로 추가 데이터 로드 (페이지당 5건, LH 필터 시 비활성화)
...
### 2025-11-10 (v2.8)
- **LH 전세임대 매칭 및 필터링 기능 추가**: 실거래가와 LH 전세임대 데이터 자동 매칭
  - **LH 데이터 매칭 로직**:
    - 시군구 코드, 계약년월, 계약일, 면적(±0.5㎡), 보증금(±5%) 조건으로 매칭
    - 4가지 주택 유형(아파트, 연립다세대, 단독다가구, 오피스텔) 모두 지원
    - 보증금 계산: LH 전세금 - LH 지원금
  - **LH 필터 체크박스**: "LH 전세임대만 보기" 체크박스로 매칭된 거래만 조회
    - 체크 시 전체 결과 한 번에 표시 (무한 스크롤 비활성화)
    - 체크 해제 시 일반 무한 스크롤 작동 (페이지당 5건)
  - **LH 뱃지 표시**:
    - 초록색 "LH" 뱃지 표시 (점선 밑줄로 상호작용 가능 표시)
    - 마우스 오버 또는 클릭 시 상세 정보 툴팁:
      - 입주자 유형 (예: 대학생, 신혼부부, 청년)
      - 방 개수 (예: 원룸, 투룸)
      - LH 지원금 (억/만원 형식)
  - **UI 개선**:
    - 메인 테이블 컬럼 너비 조정 (전월세구분 120px, 추정 호실 후보군 100px)
    - 모달 테이블 뱃지 컬럼 확대 (130px)로 LH+전월세 뱃지 한 줄 표시
    - 다른 컬럼 너비 최적화로 공간 확보
  - **성능 최적화**:
    - LH 필터 활성화 시 SQL 페이지네이션 비활성화
    - 전체 실거래가 조회 → LH 매칭 → 필터링 → 전체 반환
    - 매칭 결과가 적어 페이지네이션 불필요
  - **파일**: `app.py` (LH 매칭 로직, 필터링), `static/js/main.js` (뱃지, 툴팁), `templates/index.html` (UI, 컬럼 조정), `static/css/style.css` (LH 뱃지 스타일)

### 2025-11-07 (v2.7)
- **건물명 캐시 제거**: 초기 로딩 속도 대폭 개선
  - **변경 전**: 서버 시작 시 4개 테이블에서 건물명 캐시 로딩 (수십 초 소요)
  - **변경 후**: 캐시 로딩 제거 → 서버가 3초 이내 즉시 시작
  - `BUILDING_CACHE = {}`로 비활성화
- **건물 검색 UI 개선**: 자동완성 방식 → 검색 버튼 방식으로 변경
  - **자동완성 제거**: 입력 중 자동 검색 제거 (타이핑마다 DB 쿼리 부하)
  - **검색 버튼 추가**: 사용자가 검색 버튼을 클릭할 때만 검색 실행
  - **Enter 키 지원**: 입력창에서 Enter 키로 검색 가능
  - **그리드 레이아웃**: 검색 결과를 카드 형태의 반응형 그리드로 표시
  - **호버 효과**: 카드에 마우스 오버 시 그림자 및 이동 애니메이션
- **실시간 DB 검색**: 캐시 대신 실시간 데이터베이스 쿼리
  - 4개 테이블(아파트, 연립다세대, 오피스텔, 단독다가구)에서 실시간 검색
  - 읍면동명 + 지번 LIKE 검색으로 빠른 조회
  - 최대 12개 결과 반환 (LIMIT 적용)
- **코드 정리**: 중복 함수 제거
  - `get_db_connection()` 함수 중복 정의 제거 (app.py:239-242)
  - 에러 처리 개선 (`load_building_cache()` try-except 추가)
- **파일**: `app.py` (캐시 제거, API 수정), `templates/index.html` (UI 변경), `static/js/main.js` (검색 로직 변경), `static/css/style.css` (스타일 추가)
